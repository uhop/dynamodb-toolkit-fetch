...
**This package is a frozen re-export thunk.** The Fetch adapter's source of
truth is the core repo — `dynamodb-toolkit`'s `src/http/fetch/` (public
subpath `dynamodb-toolkit/fetch`). Do not add features here; make changes in
...
- `src/index.js` + `src/index.d.ts` — `export * from 'dynamodb-toolkit/fetch'`
- `src/read-web-body.js` + `.d.ts` — path-compat re-export of `dynamodb-toolkit/http/fetch/read-web-body.js`
