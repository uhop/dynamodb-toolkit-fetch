---
description: Pre-release verification checklist for dynamodb-toolkit-fetch
---

# Release Check

Fetch adapter release flow. Mirrors the parent `dynamodb-toolkit` release posture:
...
5. **`package.json`:**
   ...
   - `peerDependencies` covers `dynamodb-toolkit` (compatible range). No framework peer dep — Fetch is platform.
...
8. **Full check matrix:**
   ...
   npm test
   ```
   Cross-runtime (Deno / Bun) should be green — all three runtimes have native Fetch. Flag regressions in the release notes.
