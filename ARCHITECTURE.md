# Architecture — dynamodb-toolkit-fetch (frozen thunk)

Since 0.4.0 this package is a re-export thunk over the core package's
`dynamodb-toolkit/fetch` subpath. There is no implementation here:

- `src/index.js` — `export * from 'dynamodb-toolkit/fetch'` (ESM, `@ts-self-types` sidecar convention).
- `src/read-web-body.js` — path-compat re-export of `dynamodb-toolkit/http/fetch/read-web-body.js` (consumers imported it via the `./*` wildcard).
